...
epochs = 12  # 20
sc_step_size = 3  # 5
mode='std_snn'  # std_snn for pre-spike snn/ ASNN for post-spike snn
...
for i in range(large_cycle):
    ...
    for epoch in range(epochs):
        for batch_id, (audio, label) in enumerate(train_loader):
            ...
            # net.fc0.weight.grad
            # net.fc0.wx.grad
            # grad.append(net.fc0.weight.grad)
            # grad.append(net.fc1.wx)
            # grad_tensor =torch.stack(grad,dim=2).squeeze(0).squeeze(0)
            # m = grad_tensor[13, :, :]-grad_tensor[0, :, :]
            # m.any()>0
            # plt.plot(grad_tensor[13,:,:].detach().numpy())
            # a = net.fc0.wx.grad
