...
# wb = Workbook()
filename = 'E3 IC/E4 Average CC before and after IC circuit'
wb = load_workbook(f'../tab/{filename}.xlsx')
ws = wb['Sheet1']

# rows = ws.max_row
rows = [np.arange(2,8),np.arange(2,8)]
cols = [2,4]
...
# for i in range(0,2):
axe1.plot(x,result1[1,:],color=color_list[0],linestyle=linestyle_list[0],label='Before IC')
axe1.plot(x,result2[1,:],color=color_list[1],linestyle=linestyle_list[1],label='After IC')
...
fig.subplots_adjust(left=0.1,bottom=0.06,right=0.98,top=0.98,wspace=0.3,hspace=0.4)
fig.show()
# filename = '../fig/E1 Cochlea/ E1 Channels,bandwidth factor and classification accuracy.tif'
filename = '../fig/E3 IC/E3 Decreasing average CC.tif'
